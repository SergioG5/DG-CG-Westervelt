function [errDT_L2T] = getErrorDT_L2T(fem, Thx, Tht, uh, Func, nStep)
	numCells = Thx.getNumCells;
	Tx = Simplex2D();
	Tt = Simplex1D();
	cell = Cell2D();
	QW2D_ref = fem.op2D_.QW2D_;
	QX2D_ref = fem.op2D_.QX2D_;
	DPTTopQ_PX = fem.op2D_.DPTTopQ_PX_';
	Tht.getGeometry(nStep, Tt);
	tf = max(Tt.getVertices);
	hT = Tt.getDiameter;
	errDT_L2T = 0.0;
	% -------------- LOOP OVER THE ELEMENTS -------------- %
	for kCell = 1 : numCells
		Thx.getSimplex(kCell, Tx);
		Thx.getCell(kCell, cell);
		DOF = fem.getDOF(Thx, kCell);
										% ------- pde source term  --------
		QX = Tx.mapPoints(QX2D_ref);
		DT_UX = Func(QX(:, 1), QX(:, 2), tf);
		DT_UhX = (1/hT)*DPTTopQ_PX*uh(DOF.ptrsGlobalDofs);
		errDT_L2T = errDT_L2T + (Tx.getDetJac)*dot(QW2D_ref, (DT_UX - DT_UhX).^2);
	end % end of loop over the elements
	errDT_L2T = sqrt(errDT_L2T);
end  % end function

% -----------------------------------------------------------------------------
% Created by 
%
% Sergio Gomez, sergio.gomezmacias@unimib.it
% Department of Mathematics and Applications
% University of Milano-Bicocca (UNIMIB)
%
%                                   (2025)
% -----------------------------------------------------------------------------